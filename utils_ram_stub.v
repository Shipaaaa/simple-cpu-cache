    ...
    if (ram_avalid)
        if(!ram_rnw) begin // Запись в RAM
            ...
            repeat (ITER_COUNT) begin
                $display("[%3d] RAM writing %4x", $time, ram_wdata);
                data <= {ram_wdata, data[DATA_SIZE-1:WORD_SIZE]};
                data_backdoor <= {ram_wdata, data[DATA_SIZE-1:WORD_SIZE]};
                @(posedge ram_clk);
            end

            $display("[%3d] RAM next time ack", $time, ram_wdata);
            ram_ack <= 1;
            @(posedge ram_clk);
            ram_ack <= 0;
            $display("[%3d] RAM WRITE ok! addr = %0h, data = %0h", $time, ram_addr, data);
        end
        else begin
            addr = ram_addr;
            repeat(DELAY) begin
            ...
            data = {32'h70006000, 32'h50004000, 32'h30002000, 16'h1000, 4'b1001, addr};
            data_backdoor <= {32'h70006000, 32'h50004000, 32'h30002000, 16'h1000, 4'b1001, addr};
            $display("[%0d] RAM READ ok! addr = %0h, data = %0h", $time, ram_addr, data);
            repeat(ITER_COUNT) begin
                @(posedge ram_clk);
                $display("[%3d] RAM acking %4x", $time, data[WORD_SIZE-1:0]);
                ram_ack = 1;
